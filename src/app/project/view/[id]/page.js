...
export default function ProjectDetailPage() {
  ...
  const handleAddComment = async () => {
    ...
    const res = await apiRequest(`/home/projects/${project.id}/comment`, "POST", { comment: newComment });
    if (res.success) {
      setNewComment("");
      fetchProject();
    }
  ...
  const loadMoreComments = async () => {
    ...
    const res = await apiRequest(`/projects/comments?id=${project.id}&page=${nextPage}&limit=5`, "GET");
    if (res.success) {
      setProject(prev => ({
        ...prev,
        ...
        {project.images?.length > 0 && (
          <div className="flex overflow-x-auto gap-4 py-2">
            {project.images.map((img) => (
              <div key={img.id} className="relative w-80 h-48 flex-shrink-0 rounded-lg overflow-hidden">
                <Image src={img.url} alt="Project Image" fill className="object-cover rounded-lg" />
              </div>
            ))}
          </div>
        )}
          ...
          {project.hashtags?.length > 0 && (
            <div className="flex flex-wrap gap-2">
              {project.hashtags.map((tag, i) => (
                <span key={i} className="px-2 py-1 bg-gray-700 rounded text-sm">#{tag}</span>
              ))}
            </div>
          )}
        </div>
    ...
    <div>
      ...
      <p className="text-gray-400">Project Owner</p>
          ...
          <div className="flex items-center gap-4 mt-2">
            ...
            <div className="flex -space-x-3">
              {project.collaborators.map((c) => (
                <Image key={c.id} src={c.profile_image || "/default-avatar.png"} alt={c.name} width={35} height={35} className="rounded-full border-2 border-gray-900" />
              ))}
            </div>
          </div>
        )}
        ...
        <div className="flex items-center gap-4 mt-4">
          <button
            onClick={handleLike}
            className={`px-4 py-2 rounded-lg font-semibold ${project.likedByUser ? "bg-red-600 text-white" : "bg-gray-700 text-white"}`}
          >
            <Heart size={16} className="inline mr-1" /> {project.likesCount || 0} Like
          </button>

          <button
            onClick={handleCollabRequest}
            disabled={project.requestStatus === "pending" || project.requestStatus === "approved"}
            className={`px-4 py-2 rounded-lg font-semibold text-white ${project.requestStatus ? "bg-gray-600 cursor-not-allowed" : "bg-blue-600 hover:bg-blue-700"}`}
          >
 ...
 <button
        ...
        <section className="mt-6">
          ...
          <div className="flex gap-2 mb-4">
            ...
            />
            <button onClick={handleAddComment} className="px-4 py-2 bg-white text-black rounded hover:bg-gray-200">Post</button>
          </div>
          ...
          {project.comments?.length > 0 ? (
            <ul className="space-y-2">
              {project.comments.map((c) => (
                <li key={c.id} className="p-3 bg-gray-900 rounded-lg flex flex-col">
